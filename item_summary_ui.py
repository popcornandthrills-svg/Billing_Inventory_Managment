...
class ItemSummaryUI(ttk.Frame):
    ...
    def __init__(self, parent, role="admin"):
        ...
        self.sort_col_var = tk.StringVar(value="item")
        ...
        ttk.Label(top, text="Sort By").pack(side="left", padx=(6, 4))
        sort_cols = [("item", "Item Name"), ("available_qty", "Available Qty"), ("selling_price", "Selling Price")]
        if not self.restricted_view:
            sort_cols.insert(2, ("purchase_price", "Purchase Price"))
        self._sort_label_to_key = {label: key for key, label in sort_cols}
        self._sort_key_to_label = {key: label for key, label in sort_cols}
        self.sort_col_cb = ttk.Combobox(top, state="readonly", width=16, values=[label for _k, label in sort_cols])
        self.sort_col_cb.set(self._sort_key_to_label.get(self.sort_column, "Item Name"))
        self.sort_col_cb.pack(side="left", padx=(0, 6))
        self.sort_col_cb.bind("<<ComboboxSelected>>", self.on_sort_control_change)
        self.sort_order_cb = ttk.Combobox(top, state="readonly", width=12, values=["Low to High", "High to Low"])
        self.sort_order_cb.set("Low to High")
        self.sort_order_cb.pack(side="left", padx=(0, 8))
        self.sort_order_cb.bind("<<ComboboxSelected>>", self.on_sort_control_change)
        ttk.Checkbutton(
            top,
        ...
        self.tree.column("selected", anchor="center", width=70, stretch=False)
        self.tree.column("item", anchor="w", width=220, stretch=True)
        self.tree.column("available_qty", anchor="e", width=130, stretch=True)
        if not self.restricted_view:
            self.tree.column("purchase_price", anchor="e", width=130, stretch=True)
        self.tree.column("selling_price", anchor="e", width=130, stretch=True)
        ...
        self.tree.tag_configure("qty_negative", background="#fde2e2", foreground="#8b1d1d")
        self.tree.tag_configure("qty_zero", background="#fff1df", foreground="#8a4b00")
        self.tree.tag_configure("qty_positive", background="#eaf3ff", foreground="#0f3b73")
    ...
    def get_sorted_rows(self, rows):
        ...
        def key_item(r):
            return str(r.get("item", "")).lower()

        def key_num(name):
            return lambda r: self.to_float(r.get(name, 0))

        key_map = {
            "item": key_item,
            "available_qty": key_num("available_qty"),
            "purchase_price": key_num("purchase_price"),
            "selling_price": key_num("selling_price"),
        }
        return sorted(rows, key=key_map.get(col, key_item), reverse=desc)
    ...
    def sort_by(self, column):
        ...
        self.sort_order_var.set("desc" if self.sort_desc else "asc")
        if hasattr(self, "sort_col_cb"):
            self.sort_col_cb.set(self._sort_key_to_label.get(self.sort_column, "Item Name"))
        if hasattr(self, "sort_order_cb"):
            self.sort_order_cb.set("High to Low" if self.sort_desc else "Low to High")
        self.update_heading_labels()
        self.render_rows()

    def on_sort_control_change(self, _event=None):
        col_label = self.sort_col_cb.get().strip()
        self.sort_column = self._sort_label_to_key.get(col_label, "item")
        self.sort_desc = self.sort_order_cb.get().strip() == "High to Low"
        self.sort_col_var.set(self.sort_column)
        self.sort_order_var.set("desc" if self.sort_desc else "asc")
        self.update_heading_labels()
        self.render_rows()
    ...
    def render_rows(self):
        ...
        for row in self.get_filtered_rows():
            ...
            qty_value = self.to_float(row.get("available_qty", 0))
            checked = self.CHECKED_BOX if item_name in self.selected_items else self.UNCHECKED_BOX
            values = (
                checked,
                item_name,
                f"{qty_value:,.2f}",
                f"{self.to_float(row.get('selling_price', 0)):,.2f}",
            ) if self.restricted_view else (
                checked,
                item_name,
                f"{qty_value:,.2f}",
                f"{self.to_float(row.get('purchase_price', 0)):,.2f}",
                f"{self.to_float(row.get('selling_price', 0)):,.2f}",
            )

            if qty_value < 0:
                row_tag = "qty_negative"
            elif qty_value == 0:
                row_tag = "qty_zero"
            else:
                row_tag = "qty_positive"
    ...
    def open_selected_item_transactions(self):
        ...
        # Primary: use checked rows from the Select column.
        if self.selected_items:
            item_names = sorted({str(x).strip() for x in self.selected_items if str(x).strip()})
        else:
            # Fallback: use currently selected tree rows.
        ...
        if not item_names:
            messagebox.showinfo("Item Transactions", "Please select one or more item rows first.")
            return
        self.open_items_transactions(item_names)
    ...
    def open_item_transactions(self, item_name):
        self.open_items_transactions([item_name])

    def open_items_transactions(self, item_names):
        cleaned_items = [str(x).strip() for x in item_names if str(x).strip()]
        ...
        for item_name in cleaned_items:
            txns.extend(self.get_item_transactions(item_name))
        txns.sort(key=lambda r: r.get("date_sort", datetime.min), reverse=True)

        win = tk.Toplevel(self)
        title_text = cleaned_items[0] if len(cleaned_items) == 1 else f"{len(cleaned_items)} Items"
        win.title(f"Item Transactions - {title_text}")
        ...
        ttk.Label(
            win,
            text=f"Transaction Details: {title_text}",
            font=("Arial", 12, "bold")
        ...
        is_shop_manager = (self.role == "shop_manager")
        if is_shop_manager:
            columns = ("date", "type", "reference", "item", "party", "qty")
        else:
            columns = (
                "date", "type", "reference", "item", "party", "qty", "rate", "gst",
        ...
        for col in columns:
            anchor = "w" if col in ("date", "type", "reference", "item", "party", "payment_mode", "status") else "e"
        ...
        if not txns:
            ttk.Label(
                win,
                text="No matching purchase/sale transactions found for selected item(s).",
                foreground="red"
