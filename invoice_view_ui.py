...
class InvoiceViewUI(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        # ---------- LOAD ----------
        self.sales = sorted(load_sales(), key=lambda s: self._parse_date(s.get("date")), reverse=True)
        for s in self.sales:
            self.listbox.insert(
                tk.END,
                f"{s['invoice_no']} | {self._parse_date(s.get('date')).strftime('%d-%m-%Y %H:%M:%S') if self._parse_date(s.get('date')) != datetime.min else s.get('date', '')} | {s['customer_name']}"
            )
    ...
    def show_invoice(self, event=None):
        ...
        self.info.config(
            text=(
                ...
                f"Date: {self._parse_date(sale.get('date')).strftime('%d-%m-%Y %H:%M:%S') if self._parse_date(sale.get('date')) != datetime.min else sale.get('date', '')}\n"
