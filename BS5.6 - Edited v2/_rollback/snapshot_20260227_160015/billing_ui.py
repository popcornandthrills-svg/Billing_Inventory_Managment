...
from inventory import get_available_items, get_item_stock
...
class BillingUI(ttk.Frame):
    def __init__(self, parent):
        ...
        ttk.Label(add, text="Rate").grid(row=0, column=3)
        ttk.Label(add, text="GST %").grid(row=0, column=4)
        
        self.item_cb = ttk.Combobox(add, state="readonly", width=22)
    ...
    def show_customer_suggestions(self, event=None):
        ...
        matches = [
            c["name"] for c in customers.values()
    ...
    def refresh_items(self):
        ...
        if items:
            self.item_cb.set(items[0])

    def autofill_by_phone(self, event=None):
        c = get_customer_by_phone(self.phone.get().strip())
        if c:
            self.cust_name.delete(0, tk.END)
    ...
    def autofill_by_name(self, event=None):
        c = get_customer_by_name(self.cust_name.get().strip())
        if c:
            self.phone.delete(0, tk.END)
            self.phone.insert(0, c["phone"])
            self.address.delete(0, tk.END)
    ...
    def refresh_total(self):
        ...
        self.grand_total = taxable + cgst + sgst
