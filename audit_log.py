...
def write_audit_log(
    ...
    effective_user = user
    # If caller passes no user or a hardcoded admin identity, prefer current logged-in user context.
    if CURRENT_AUDIT_USER and (
        not effective_user
        or str(effective_user).strip().lower() in ("admin", "admin123")
    ):
