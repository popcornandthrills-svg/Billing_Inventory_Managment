...
def adjust_item_summary_available_qty(item_name, delta_qty):
    """
    If an available_qty override exists for item_name, adjust it by delta_qty.
    Keeps manual override mode in sync with real stock movements.
    """
    key = normalize_item_name(item_name)
    if not key:
        return
    ...
    if not isinstance(rec, dict):
        return
    if "available_qty" not in rec:
        return

    rec["available_qty"] = round(to_float(rec.get("available_qty", 0)) + to_float(delta_qty), 2)
    overrides[key] = rec
    save_json(OVERRIDES_FILE, overrides)
