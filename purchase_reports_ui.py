...
class PurchaseReportsUI(ttk.Frame):
    ...
    def build_ui(self):
        ...
        self.selected_summary_var = tk.StringVar(value="Selected: 0 | Amount: 0.00")
    ...
    def on_selection_change(self, _event=None):
        ...
        if not selected:
            self.selected_summary_var.set("Selected: 0 | Amount: 0.00")
            return
        amount = 0.0
        for iid in selected:
            ...
            if not vals:
                continue
            try:
                amount += float(vals[3] or 0)
            except Exception:
                continue
        self.selected_summary_var.set(f"Selected: {len(selected)} | Amount: {amount:.2f}")
