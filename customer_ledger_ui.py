...
class CustomerLedgerUI(ttk.Frame):
    ...
    def build_ui(self):
        ...
        self.phone_e.bind("<KeyRelease>", self.on_phone_change)
    ...
    def load_ledger(self):
        ...
        from_raw = self.from_date_e.get().strip()
        to_raw = self.to_date_e.get().strip()
        has_from = bool(from_raw and from_raw not in ("DD-MM-YYYY", "YYYY-MM-DD"))
        has_to = bool(to_raw and to_raw not in ("DD-MM-YYYY", "YYYY-MM-DD"))
        if not any([name, phone, item, has_from, has_to]):
            messagebox.showerror(
                "Input Required",
                "Enter at least one filter (Customer Name, Phone, Item, From Date, or To Date) to load ledger."
            )
            self.name_e.focus_set()
            return

        from_date = self.parse_date(from_raw)
        to_date = self.parse_date(to_raw)
    ...
    def _normalize_phone(self):
        raw = "".join(ch for ch in self.phone_e.get() if ch.isdigit())[:10]
        if raw != self.phone_e.get():
            self.phone_e.delete(0, tk.END)
            self.phone_e.insert(0, raw)
        return raw

    def on_phone_change(self, _event=None):
