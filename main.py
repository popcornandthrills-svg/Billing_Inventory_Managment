...
class App(tk.Tk):
    def __init__(self):
        ...
        sw = max(self.winfo_screenwidth(), 900)
        sh = max(self.winfo_screenheight(), 620)
        ...
        try:
            ...
            pass
        setup_style(self)
        self.bind_all("<KeyRelease>", self._auto_capitalize_first_letter, add="+")
    ...
    def _auto_capitalize_first_letter(self, event=None):
        widget = (event or object()).widget if event else None
        if widget is None:
            return
        try:
            ...
            first = text[0]
            if first.isalpha():
                capitalized = first.upper() + text[1:]
                if capitalized != text:
...
class LoginFrame(ttk.Frame):
    ...
    def check_login(self):
        ...
        if pwd == ADMIN_PASSWORD:
            ...
            set_current_audit_user(audit_identity)
            ...
            return
        # Keep context in sync for all following actions in this session.
...
class DashboardFrame(ttk.Frame):
    def __init__(self, parent, app):
        ...
        ttk.Label(
            header,
            text="Goldprince Trade Centre LTD",
            style="Header.TLabel"
        ).pack()
        ...
        # LEFT
        self.left = ttk.Frame(main, width=230)
    ...
    def refresh(self):
        ...
        self._add_nav_button("due_report", "Sales Due Report", self.open_due_report)
    ...
    def _add_nav_button(self, key, text, command, pady=4):
        ...
        btn.pack(fill="x", pady=pady, padx=(20, 0), anchor="w")
    ...
    def _on_window_resize(self, _event=None):
        try:
            ...
            left_w = min(max(int(total_w * 0.22), 200), 280)
            self.left.configure(width=left_w)
        except Exception:
            pass
    ...
    def _switch_view(self, builder, cache_key=None):
        ...
        def _build():
            try:
                ...
                messagebox.showerror("Open Error", f"Could not open section.\n\n{e}")
        self.after(1, _build)
    ...
    def open_customer_report(self):
        # Customer Ledger is merged into Sales Due Report.
        self.open_due_report()
    ...
    def open_manage_sm_accounts(self):
        def _build():
            ...
            def _select_row_from_event(event):
                ...
                tree.focus(row_id)
                _on_select()
                return _selected_username()

            def _open_manage_options(event):
                uname = _select_row_from_event(event)
                if not uname:
                    return
                menu = tk.Menu(tree, tearoff=0)
                menu.add_command(label=f"Manage: {uname}", state="disabled")
                menu.add_separator()
                menu.add_command(label="View Details", command=lambda: _load_user_details(uname))
                menu.add_command(
                    label="Reset Password",
                    command=lambda: (reset_password.focus_set(), status_var.set(f"Enter new password for: {uname}")),
                )
                menu.add_command(label="Delete SM", command=_delete_selected)
                menu.add_separator()
                menu.add_command(label="Refresh List", command=_load_accounts)
                try:
            ...
            tree.bind("<Double-1>", _open_manage_options)
            tree.bind("<Button-3>", _open_manage_options)
            ...
            ttk.Button(form, text="Delete SM", style="Danger.TButton", command=_delete_selected).grid(
